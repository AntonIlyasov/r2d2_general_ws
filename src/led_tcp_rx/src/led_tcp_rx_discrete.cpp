...
uint32_t countClbFromLedControl = 0;
...
#define DATA_TO_LED_CONTROL_TOPIC_SIZE         1  // [1b DATA]
#define DATA_FROM_LED_CONTROL_TOPIC_SIZE       2  // [1b DATA][OK]

#define TO_LED_CONTROL_TOPIC_NAME       "toLedControlTopic"
#define FROM_LED_CONTROL_TOPIC_NAME     "fromLedControlTopic"
...
public:
  ...
  : socket_(io_context) {
    toLedControlPub   = node.advertise<std_msgs::ByteMultiArray>(TO_LED_CONTROL_TOPIC_NAME, 0);
    fromLedControlSub = node.subscribe<std_msgs::ByteMultiArray>(FROM_LED_CONTROL_TOPIC_NAME, 0, 
        &Session::from_led_control_callback, this);

    memset(dataFromTCP, 0, sizeof(dataFromTCP));
    memset(dataToTCP, 0, sizeof(dataToTCP));
    memset(dataToLedControl, 0, sizeof(dataToLedControl));
    memset(dataFromLedControl, 0, sizeof(dataFromLedControl));
  }
...
private:
  ...
  uint8_t dataToLedControl[DATA_TO_LED_CONTROL_TOPIC_SIZE];
  uint8_t dataFromLedControl[DATA_FROM_LED_CONTROL_TOPIC_SIZE];

  // остальные переменные
  ros::Publisher toLedControlPub;
  ros::Subscriber fromLedControlSub;
  
  uint32_t send_count_TCP                     = 0;
  uint32_t recvd_count_TCP                    = 0;
  uint32_t send_count_led_control             = 0;
  uint32_t recvd_count_led_control            = 0;
  uint32_t resvdFailCount                     = 0;
  uint32_t sendFailCount                      = 0;
  uint8_t  resvdBytesFromLedControl           = 0;
  
  struct currentState_{
    uint8_t keepalive[4]                      = {0};
    uint8_t from_led_control_command          = 0;
    uint8_t from_led_control_OK               = 0;
  };
  ...
  // обработчик сообщений от ROS-топика с Led Control. 
  // полученные данные с ROS-топика "расфасовываются" в переменные для дальнейшей отправки по TCP
  void from_led_control_callback(const std_msgs::ByteMultiArray::ConstPtr& recvdMsg){
    countClbFromLedControl++;
    if (countClbFromLedControl == 2) return;
    std::cout << "CallBack!!!! countClbFromLedControl = " << countClbFromLedControl;
    recvd_count_led_control++;
    resvdBytesFromLedControl = recvdMsg->data.size();
    
    std::cout << "\n\033[1;34mRECVD FROM TOPIC fromLedControlTopic resvdBytesFromLedControl = \033[0m" 
        << resvdBytesFromLedControl << std::endl;
    std::cout << "recvd_count_led_control = " << recvd_count_led_control << std::endl;

    if (recvdMsg->data.size() == DATA_FROM_LED_CONTROL_TOPIC_SIZE){
      for (int i = 0; i < recvdMsg->data.size(); i++){
        dataFromLedControl[i] = recvdMsg->data[i];
        printf("[%u]", dataFromLedControl[i]);
      }
      std::cout << std::endl;
      currentState.from_led_control_command  = dataFromLedControl[0];
      currentState.from_led_control_OK       = dataFromLedControl[1];
  ...
  */
  ...
  void sendMsgToLedControl(){
    //отправка пакета в топик "toLedControlTopic"
    std_msgs::ByteMultiArray msg;
    msg.layout.dim.push_back(std_msgs::MultiArrayDimension());
    msg.layout.dim[0].size = 1;
    msg.layout.dim[0].stride = sizeof(dataToLedControl);
    msg.data.clear();

    send_count_led_control++;
    std::cout << "\nsend_count_led_control = " << send_count_led_control << std::endl;
    std::cout << "\033[1;34mSEND T0 toLedControlTopic: \033[0m";

    for (int i = 0; i < sizeof(dataToLedControl); i++) {
      printf("[%u]", dataToLedControl[i]);
      msg.data.push_back(dataToLedControl[i]);
    }
    std::cout << std::endl;
    
    toLedControlPub.publish(msg);
  }

  // отправка полученных данных с ROS-топикa от Led Control пользователю по протоколу TCP
  void sendMsgToTCP(){

    // формируем пакет данных для отправки по протоколу TCP
    if (resvdBytesFromLedControl == DATA_FROM_LED_CONTROL_TOPIC_SIZE){
      dataToTCP[0] = 0xBB;  
      dataToTCP[1] = 0xAA;  
      dataToTCP[2] = sizeof(dataToTCP);
      dataToTCP[3] = currentState.from_led_control_command;
      dataToTCP[4] = currentState.from_led_control_OK;
  ...
  /*
    ...
    для дальнейшей отправки ROS-топик на Led Control.
  */
      ...
      size_t bytes_transferred)
  {
    if (!error)
    {
      ...
      memcpy(dataToLedControl, &dataFromTCP[3], sizeof(dataToLedControl));
      ...
      size_t bytes_transferred)
  {
    if (!error && bytes_transferred > 0)
    {
      ...
      std::cout << "\n!!!OK!!!\n";
      currentState.from_led_control_OK = 0;
      send_count_TCP++;
...
int main(int argc, char* argv[])
{
  try{
    std::cout << "\n\033[1;32m╔═══════════════════════════════╗\033[0m"
              << "\n\033[1;32m║     led_tcp_rx is running!    ║\033[0m" 
              << "\n\033[1;32m╚═══════════════════════════════╝\033[0m\n";
    ros::init(argc, argv, "led_tcp_rx");
