...
#define DATA_TO_LED_CONTROL_TOPIC_SIZE         1  // [1b]
#define DATA_FROM_LED_CONTROL_TOPIC_SIZE       2  // [1b][OK]
...
public:
  UDPServer(boost::asio::io_service& io_service): socket_(io_service, udp::endpoint(udp::v4(), PORT)){
    ...
    fromLedControlSub = node.subscribe<std_msgs::ByteMultiArray>(FROM_LED_CONTROL_TOPIC_NAME, 0, 
        &UDPServer::from_led_control_callback, this);

    memset(dataFromUDP, 0, sizeof(dataFromUDP));
    memset(dataToUDP, 0, sizeof(dataToUDP));
    memset(dataToLedControlTopic, 0, sizeof(dataToLedControlTopic));
    memset(dataFromLedControlTopic, 0, sizeof(dataFromLedControlTopic));
...
private:
  ...
  uint8_t dataToLedControlTopic[DATA_TO_LED_CONTROL_TOPIC_SIZE];
  uint8_t dataFromLedControlTopic[DATA_FROM_LED_CONTROL_TOPIC_SIZE];
  ...
  uint32_t send_count_topic_led_control       = 0;
  uint32_t recvd_count_topic_led_control      = 0;
  ...
  void from_led_control_callback(const std_msgs::ByteMultiArray::ConstPtr& recvdMsg){
    ...
    recvd_count_topic_led_control++;
    resvdBytesFromLedControl = recvdMsg->data.size();
    
    std::cout << "\n\033[1;34mRECVD FROM TOPIC fromLedControlTopic resvdBytesFromLedControl = \033[0m" << resvdBytesFromLedControl << std::endl;
    std::cout << "recvd_count_topic_led_control = " << recvd_count_topic_led_control << std::endl;

    if (recvdMsg->data.size() == DATA_FROM_LED_CONTROL_TOPIC_SIZE){
      for (int i = 0; i < recvdMsg->data.size(); i++){
        dataFromLedControlTopic[i] = recvdMsg->data[i];
        printf("[%u]", dataFromLedControlTopic[i]);
      }
      std::cout << std::endl;
      currentState.from_led_control_command  = dataFromLedControlTopic[0];
      currentState.from_led_control_OK       = dataFromLedControlTopic[1];
    }
  }
  ...
  */
  void udp_handle_receive(const boost::system::error_code& error, size_t bytes_transferred) {
    ...
    getMsgFromUDP = true;
    memcpy(dataToLedControlTopic,  &dataFromUDP[3], sizeof(dataToLedControlTopic));
  ...
  void sendMsgToLedControl(){
    ...
    msg.layout.dim[0].stride = sizeof(dataToLedControlTopic);
    msg.data.clear();
    std::cout << "\033[1;34mSEND T0 toLedControlTopic: \033[0m";

    for (int i = 0; i < sizeof(dataToLedControlTopic); i++) {
      printf("[%u]", dataToLedControlTopic[i]);
      msg.data.push_back(dataToLedControlTopic[i]);
    }
    std::cout << std::endl;
  ...
  // отправка полученных данных с ROS-топикa от Led Control пользователю по протоколу UDP
  void sendMsgToUDP(){
    ...
    if (resvdBytesFromLedControl == DATA_FROM_LED_CONTROL_TOPIC_SIZE){
      ...
      dataToUDP[3] = currentState.from_led_control_command;
      dataToUDP[4] = currentState.from_led_control_OK;
...
int main(int argc, char* argv[])
{
  try{
    std::cout << "\n\033[1;32m╔═══════════════════════════════╗\033[0m"
              << "\n\033[1;32m║     led_udp_rx is running!    ║\033[0m" 
              << "\n\033[1;32m╚═══════════════════════════════╝\033[0m\n";
    ros::init(argc, argv, "led_udp_rx");
